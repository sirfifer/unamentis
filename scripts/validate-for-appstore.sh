#!/bin/bash
#
# VoiceLearn App Store Validation Script
...
ARCHIVE_PATH="$BUILD_DIR/VoiceLearn.xcarchive"
...
echo ""
echo -e "${GREEN}╔════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║     VoiceLearn App Store Validation                            ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
# Privacy Manifest
if [ -f "VoiceLearn/PrivacyInfo.xcprivacy" ]; then
  print_check "PrivacyInfo.xcprivacy exists"

  # Validate XML
  if plutil -lint VoiceLearn/PrivacyInfo.xcprivacy > /dev/null 2>&1; then
...
# Info.plist
if [ -f "VoiceLearn/Info.plist" ]; then
  print_check "Info.plist exists"

  # Check required keys
  MICROPHONE_DESC=$(plutil -extract NSMicrophoneUsageDescription raw VoiceLearn/Info.plist 2>/dev/null || echo "")
  ...
  SPEECH_DESC=$(plutil -extract NSSpeechRecognitionUsageDescription raw VoiceLearn/Info.plist 2>/dev/null || echo "")
...
# Entitlements
if [ -f "VoiceLearn/VoiceLearn.entitlements" ]; then
  print_check "VoiceLearn.entitlements exists"
else
  print_warning "VoiceLearn.entitlements not found"
  ((WARNINGS++))
fi

# App Icons
if [ -d "VoiceLearn/Assets.xcassets/AppIcon.appiconset" ]; then
  ICON_COUNT=$(ls VoiceLearn/Assets.xcassets/AppIcon.appiconset/*.png 2>/dev/null | wc -l | tr -d ' ')
...
if grep -rE "(sk-[a-zA-Z0-9]{20,}|AKIA[0-9A-Z]{16})" --include="*.swift" VoiceLearn/ 2>/dev/null | grep -v "Test" | grep -v "Mock" | grep -v "example" > /dev/null; then
...
if grep -q "#if DEBUG" VoiceLearn/Core/Logging/RemoteLogHandler.swift 2>/dev/null; then
...
PRINT_COUNT=$(grep -rn "print(" --include="*.swift" VoiceLearn/ 2>/dev/null | wc -l | tr -d ' ')
...
if [ "$CREATE_ARCHIVE" = true ]; then
  ...
  if [ -d "$ARCHIVE_PATH" ]; then
    ...
    # Get size
    APP_SIZE=$(du -sh "$ARCHIVE_PATH/Products/Applications/VoiceLearn.app" 2>/dev/null | cut -f1)
