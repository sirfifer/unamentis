...
const FIELD_HELP = {
  ...
  segmentType:
    'The purpose of this segment: introduction, lecture, explanation, example, checkpoint, transition, or summary.',
  segmentContent: 'The actual spoken content. Write conversationally as if speaking to a student.',
  media:
    'Visual assets (images, diagrams) to display during the lesson. Can be timed to specific segments.',
  ...
  displayMode:
    'How the media appears: persistent (stays visible), highlight (emphasized), or popup (modal).',
};
...
export const NodeEditor: React.FC<NodeEditorProps> = ({ node, onChange, readOnly }) => {
  ...
  const updateMediaTiming = (
    id: string,
    field: 'startSegment' | 'endSegment' | 'displayMode',
    value: number | string
  ) => {
    if (!node.media?.embedded) return;
    onChange({
  ...
  return (
    <div className="h-full flex flex-col bg-slate-900 text-slate-200">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <select
                    value={node.type}
                    onChange={(e) =>
                      handleFieldChange('type', e.target.value as ContentNode['type'])
                    }
                    disabled={readOnly}
          ...
          <div className="space-y-6 max-w-3xl animate-in">
            ...
            <div className="space-y-4">
              {(node.transcript?.segments || []).map((seg, idx) => (
                <div
                  key={seg.id}
                  className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4 flex gap-4 group hover:border-indigo-500/30 transition-all"
                >
                  ...
                  {!readOnly && (
                    <button
                      onClick={() => {
                        const updated =
                          node.transcript?.segments.filter((s) => s.id !== seg.id) ?? [];
            ...
            <div className="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 rounded-xl p-6">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-sm text-slate-400">
                    Manage images, diagrams, formulas, and maps that appear during the lesson.
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(node.media?.embedded || []).map((media) => (
                  <div
                    key={media.id}
                    className="bg-slate-900 border border-slate-700 rounded-xl overflow-hidden group hover:border-indigo-500/50 transition-all hover:shadow-xl"
                  >
                    <div className="relative aspect-video bg-slate-800">
                      ...
                      <div className="absolute top-2 left-2 bg-black/60 backdrop-blur px-2 py-1 rounded text-xs font-medium text-white">
                        Segments {media.segmentTiming?.startSegment} -{' '}
                        {media.segmentTiming?.endSegment}
                      </div>
                    </div>
                      ...
                      <div className="bg-slate-800/50 rounded-lg p-3 space-y-3">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          <div className="flex-1">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updateMediaTiming(media.id, 'endSegment', parseInt(e.target.value))
                              }
                              className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-xs text-white"
                            />
                          </div>
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              updateMediaTiming(media.id, 'displayMode', e.target.value)
                            }
                            className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-xs text-white"
                          >
                ...
                {(!node.media?.embedded || node.media.embedded.length === 0) && (
                  ...
                  >
                    ...
                    <p className="text-slate-500 text-sm mt-1">
                      Click to browse the creative commons library
                    </p>
                  </div>
                )}
              </div>
      ...
      {showMediaPicker && (
        <MediaPicker onSelect={handleAddMedia} onClose={() => setShowMediaPicker(false)} />
      )}

      {showDiagramEditor && (
        <DiagramEditor onSave={handleAddDiagram} onClose={() => setShowDiagramEditor(false)} />
      )}

      {showFormulaEditor && (
        <FormulaEditor onSave={handleAddFormula} onClose={() => setShowFormulaEditor(false)} />
      )}

      {showMapEditor && <MapEditor onSave={handleAddMap} onClose={() => setShowMapEditor(false)} />}
    </div>
  );
};
